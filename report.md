...
图形化操作：提供图形界面，包含钱包、交易、挖矿、链浏览和余额查询等标签页，降低学习成本。

背景知识
椭圆曲线与ECDSA比特币等加密货币使用椭圆曲线数字签名算法（ECDSA）来生成密钥对并验证签名。在比特币中使用的曲线是secp256k1，它由国际标准《SEC》定义。该曲线具有高效计算的特性，利用其生成的私钥和公钥可在不泄露私钥的情况下证明某人拥有相应地址。
数字签名系统允许用户生成私钥/公钥对，然后利用私钥对消息生成签名，签名可证明持有者拥有对应公钥。在比特币网络中，任何人都可以生成自己的密钥对，其他人可以向该公钥生成的地址发送资金，但只有拥有正确私钥的人才能签署交易并花费这些资金。

地址生成
在真实的比特币或以太坊网络中，地址是公钥的短形式，通常通过对公钥进行哈希并截断来生成。在以太坊中，公钥经过Keccak‑256哈希后取右侧20字节（40个十六进制字符）并加上“0x”前缀作为地址
。这种方法确保地址与公钥唯一对应，而且由于地址是公钥的截断哈希，无法通过地址反推出公钥。比特币地址通常使用双重哈希（SHA‑256和RIPEMD‑160）产生160位散列。本项目为了简化实现，仅对公钥做SHA‑256哈希并截取前20字节形成地址。
...
地址生成：函数address_from_public将公钥用X9.62非压缩格式序列化，然后用SHA‑256散列，取前40个十六进制字符（20字节）作为地址。这与以太坊取Keccak‑256散列后保留20字节形成地址的思想类似。由于未进行Base58或Bech32编码，该地址仅用于本项目内部测试。
...
结论
通过分析现有实现，我认识到简化假设与真实系统之间的差距，例如地址编码、UTXO模型、默克尔树和网络共识等。后续工作将围绕这些方面展开，逐步提升系统的完整性和安全性。
